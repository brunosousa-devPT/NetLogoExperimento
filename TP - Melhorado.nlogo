...
to setup-turtles
  ...
  if Transformadores? [
    create-Transformadores nTransformadores [
       set shape "turtle"
  set color white
  setxy random-xcor random-ycor
    while [[pcolor] of patch-here != black]
...
to Go
  ...
  ask Transformadores [Transformadoresdie]
...
;Funcao para o movimento dos transformadores
to moveTransformadores
  ask Transformadores [
  ifelse random 101 >= 20  and patch-ahead 1 != nobody [
    fd 1
  ]
  ...
  if [pcolor] of patch-here = yellow or [pcolor] of patch-here = red [TransformarResiduo]
...
to TransformarResiduo

  if [pcolor] of patch-here = yellow [ set trocas trocas - 1]
  if [pcolor] of patch-here = red [set trocas trocas - 2]
  set pcolor green
end

to TransformadoresDie
  if trocas < 1 [die]
...
to moveLimpadores
  ...
  [
    ...
    if ShareResiduos? [shareResiduos]
...
to shareResiduos
  ...
  if any? Limpadores-on neighbors [
      if deposito > nLimTrade [ ;Limtrada [6-10]
        set variavel deposito
        ask one-of Limpadores-on neighbors [
          if deposito < 5  [
            set variavel round(variavel - deposito)
            set arredonda (ceiling(variavel / 2) - (variavel / 2))
            set arredonda ceiling(arredonda)

            set deposito deposito + (ceiling(variavel / 2) + arredonda)
...
"" ""
PENS
"limpadores" 1.0 0 -4079321 true "" "plot count Limpadores"
"Comilões" 1.0 0 -15575016 true "" "plot count comiloes"
...
"Resíduo normal" 1.0 0 -4079321 true "" "plot count patches with [pcolor = yellow]"
...
<experiments>
  <experiment name="experiment1" repetitions="10" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="Transformadores?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="nTransformadores">
      <value value="2"/>
      <value value="5"/>
      <value value="10"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="ntrocas">
      <value value="8"/>
    </enumeratedValueSet>
  ...
  <experiment name="experiment3" repetitions="10" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="ShareResiduos?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="nLimTrade">
      <value value="6"/>
      <value value="8"/>
      <value value="10"/>
    </enumeratedValueSet>
